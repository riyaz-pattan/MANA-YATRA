...
class _DriversScreenState extends ConsumerState<DriversScreen> {
  ...
  final Map<String, Map<String, dynamic>> _localMutations = {};
  ...
  Future<void> _fetchCounts() async {
    try {
      ...
      final blkSnap = await db
          .where('isBlocked', isEqualTo: true)
          .count()
          .get();
      final apprSnap = await db
  ...
  void _exportPdf() async {
    ...
    final selectedDocs = _drivers
        .where((d) => _selectedDrivers.contains(d.id))
        .toList();
    final dataList = selectedDocs
            ...
            child: Row(
              children: [
                ...
                _FilterTab(
                  ...
                  value: 'pending',
                  groupValue: _filterStatus,
                  onTap: () => _setFilterStatus('pending'),
                ),
                const SizedBox(width: 8),
                _FilterTab(
                  label: 'Approved',
                  ...
                  value: 'approved',
                  groupValue: _filterStatus,
                  onTap: () => _setFilterStatus('approved'),
                ),
                const SizedBox(width: 8),
                _FilterTab(
                  label: 'Rejected',
              ...
              child: SingleChildScrollView(
                ...
                child: ConstrainedBox(
                  constraints: BoxConstraints(
                    minWidth: width - (isDesktop ? 300 : 32),
                  ),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      DataTable(
                        ...
                        rows: _drivers.map((doc) {
                          ...
                          final mutation = _localMutations[doc.id] ?? {};
                          final data = {...originalData, ...mutation};
  ...
  DataRow _buildDataRow(
    ...
    return DataRow(
      ...
      onSelectChanged: (val) {
        setState(() {
          if (val == true)
            _selectedDrivers.add(id);
          else
            _selectedDrivers.remove(id);
        });
      },
      cells: [
        // Driver Info
                    ...
                    children: [
                      Text(
                        ...
                        style: GoogleFonts.inter(
                          ...
                          color: AppTheme.brandBlue, // Indicate clickability
                        ),
                      ),
                      Text(
                        phone,
