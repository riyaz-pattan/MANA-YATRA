...
void main() {
  ...
  group('Rider Happy Path Core Flow', () {
    testWidgets('Complete ride flow from booking to collecting cash', (tester) async {
      app.main();
      ...
      expect(activeRides.docs.isNotEmpty, true, reason: 'Ride should be created in Firestore');
      ...
      await FirebaseFirestore.instance.collection('rides').doc(rideId).update({'status': 'arrived'});
      await tester.pumpAndSettle(const Duration(seconds: 2));
      
      await FirebaseFirestore.instance.collection('rides').doc(rideId).update({'status': 'started'});
      await tester.pumpAndSettle(const Duration(seconds: 2));

      // 8. Simulate Driver Completing Ride
      await FirebaseFirestore.instance.collection('rides').doc(rideId).update({'status': 'completed'});
      await tester.pumpAndSettle(const Duration(seconds: 3));
